package com.example.applock.ui.Adapter
...
class LookedAPPS(private val context: Context, initialChecked: ArrayList<String> = arrayListOf()) : RecyclerView.Adapter<LookedAPPS.AppViewHolder>()  {
        ...
        context.packageManager.getInstalledApplications(PackageManager.GET_META_DATA).sortedBy { it.loadLabel(packageManager).toString() }.forEach { info ->
            if (info.packageName != context.packageName) {
                if (packageManager.getLaunchIntentForPackage(info.packageName) != null) {
                    ...
                    Log.v("DataReceive","DATA")
                   // appList.add(info)


                     if (initialChecked.contains(info.packageName))
                   {
                       appList.add(info)
                   }
    ...
    fun getCheckedApps1(): ArrayList<String> {
